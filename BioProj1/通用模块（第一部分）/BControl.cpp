...
bool CBControl::ItemChecked( int index ) const
{
	if ( IsClassName( TEXT("SysListView32") ) )
	{
		UINT ret = SendMessage(m_hWnd, LVM_GETITEMSTATE, index-1, LVIS_STATEIMAGEMASK); // -1 转换为从 0 开始
...
void CBControl::ItemCheckedSet( int index, bool bSel )
{
	if ( IsClassName( TEXT("SysListView32") ) )
	{
		LVITEM itm;
		itm.mask = LVIF_STATE;
		itm.stateMask = LVIS_STATEIMAGEMASK;
		itm.iItem = index - 1;			// 转换为从 0 开始
		itm.iSubItem = 0;
		if (bSel) itm.state = 2; else itm.state = 1;
		itm.state = itm.state <<12; // * 4096  Bits 12 through 15 of this member specify the state image index
		SendMessage(m_hWnd, LVM_SETITEMSTATE, index-1, (LPARAM)(&itm));  // -1 转换为从 0 开始
	}
	// 该控件不能用 ItemCheckedSet
}



int CBControl::ItemImage( int indexItem, int indexSubItem/*=1*/ ) const
{
	if ( IsClassName( TEXT("SysListView32") ) )
	{
		LVITEM itm;
		itm.mask = LVIF_IMAGE;
		itm.iItem = indexItem - 1;			// 转换为从 0 开始
		itm.iSubItem = indexSubItem - 1;	// 转换为从 0 开始
		SendMessage(m_hWnd, LVM_GETITEM, 0, (LPARAM)(&itm));
		return itm.iImage + 1;
	}
	//else if ( IsClassName( TEXT("SysTreeView32") ) )
	//{
	//	TVITEM tvitm;
	//	SendMessage(m_hWnd, TVM_GETITEM, 0, (LPARAM)(&tvitm));
	//}
...
void CBControl::ItemImageSet( int indexItem, int indexSubItem/*=1*/, int iImage/*=0*/ )
{
	if ( IsClassName( TEXT("SysListView32") ) )
	{
		LVITEM itm;
		...
		SendMessage(m_hWnd, LVM_UPDATE, indexItem - 1, 0);
	}
	//else if ( IsClassName( TEXT("SysTreeView32") ) )
	//{
	//	TVITEM tvitm;
	//	SendMessage(m_hWnd, TVM_SETITEM, 0, (LPARAM)(&itm));
	//}
	// 该控件不能用 ItemImageSet
...
RECT CBControl::ListViewGetItemRect( int indexItem, int indexSubItem /*= 1*/, HWND hwndControlToMove /*= NULL*/, ELsvGetItemRectCode rectType /*= lsvir_Label*/ )
{
	RECT rc;
	rc.left=0; rc.right=0; rc.top=0; rc.bottom=0;  // 清零
	rc.left = (LONG)rectType;	// When the message is sent, the left member of this structure is used to
								// determine the portion of the list view item for which to retrieve the bounding rectangle.
	if (indexSubItem>1)
	{
		rc.top = (LONG)indexSubItem-1; // The one-based index of the subitem.
		SendMessage(m_hWnd, LVM_GETSUBITEMRECT, indexItem - 1, (LPARAM)(&rc));
	}
	else
	{
		SendMessage(m_hWnd, LVM_GETITEMRECT, indexItem - 1, (LPARAM)(&rc));
	}

	if (hwndControlToMove)
	{
		// 移动指定控件到该项目的范围
		//（支持该控件和 ListView 分别位于不同容器）。本函数可自动控制该控件不会越界其父窗口的客户区
		POINT pt1, pt2;

		// 获得要移动到的位置，并将其转换为屏幕坐标 => pt1, pt2
		pt1.x = rc.left + 1; pt1.y = rc.top;
		pt2.x = rc.right; pt2.y = rc.bottom - 1;
		ClientToScreen(m_hWnd, &pt1);
		ClientToScreen(m_hWnd, &pt2);

		// 将要移动到的位置（屏幕坐标）转换为 hwndControlToMove 的父窗口的坐标
		HWND hWndParent = GetParent(hwndControlToMove);
		if (hWndParent) { ScreenToClient(hWndParent, &pt1); ScreenToClient(hWndParent, &pt2); }
		// 如果pt1、pt2越界，控制其范围
		RECT rcParentCl;
		GetClientRect (hWndParent, &rcParentCl);

		if (pt2.x > rcParentCl.right)  pt2.x = rcParentCl.right;  // rcParentCl.left、top 总为0
		if (pt2.y > rcParentCl.bottom) pt2.y = rcParentCl.bottom;
		if (pt1.x < 0)  pt1.x = 0;
		if (pt1.y < 0)  pt1.y = 0;

		// 移动控件
		MoveWindow ( hwndControlToMove, pt1.x, pt1.y, pt2.x - pt1.x, pt2.y - pt1.y, 1);
	}
...
int CBControl::ListViewColumnAlignment( int indexCol )
{
	LVCOLUMN col;
	col.mask = LVCF_FMT;
	SendMessage(m_hWnd, LVM_GETCOLUMN, indexCol - 1, (LPARAM)(&col) );
...
void CBControl::ListViewColumnAlignmentSet( int indexCol, int iAlign /*= 0*/ )
{
	LVCOLUMN col;
	col.mask = LVCF_FMT;
	col.fmt = iAlign;	// LVCFMT_LEFT==0; LVCFMT_RIGHT==1; LVCFMT_CENTER=2
	SendMessage(m_hWnd, LVM_SETCOLUMN, indexCol - 1, (LPARAM)(&col) );
}
...
bool CBControl::ListViewAllowSubItemImages()
{
	if (SendMessage(m_hWnd, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0) & LVS_EX_SUBITEMIMAGES)
		return true;
	else
		return false;
}

void CBControl::ListViewAllowSubItemImagesSet( bool blNewValue )
{
	if (blNewValue)
		SendMessage (m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_SUBITEMIMAGES, LVS_EX_SUBITEMIMAGES);    
	else
		SendMessage (m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_SUBITEMIMAGES, 0);
...
bool CBControl::ListViewCheckBoxes()
{
	if (SendMessage(m_hWnd, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0) & LVS_EX_CHECKBOXES)
		return true;
	else
		return false;
}

void CBControl::ListViewCheckBoxesSet( bool blNewValue )
{
	if (blNewValue)
		SendMessage (m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_CHECKBOXES, LVS_EX_CHECKBOXES);  
	else
		SendMessage (m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_CHECKBOXES, 0);  
}



HIMAGELIST CBControl::hImageListCtrl( EImageListCtrlType imlType )
{
	if ( IsClassName( TEXT("SysListView32") ) )
		return (HIMAGELIST)SendMessage ( m_hWnd, LVM_GETIMAGELIST, (WPARAM)imlType, 0);
	else if ( IsClassName( TEXT("SysTreeView32") ) )
		return (HIMAGELIST)SendMessage ( m_hWnd, TVM_GETIMAGELIST, (WPARAM)imlType, 0);
	else
		return NULL;
}

void CBControl::hImageListCtrlSet( EImageListCtrlType imlType, HIMAGELIST hImgList )
{
	if ( IsClassName( TEXT("SysListView32") ) )
		SendMessage ( m_hWnd, LVM_SETIMAGELIST, (WPARAM)imlType, (LPARAM)hImgList);
	else if ( IsClassName( TEXT("SysTreeView32") ) )
		SendMessage ( m_hWnd, TVM_SETIMAGELIST, (WPARAM)imlType, (LPARAM)hImgList);
}
...
CBImageList::~CBImageList()
{
	if (m_ImgIdxes) { delete []m_ImgIdxes; m_ImgIdxes=NULL; m_ImgIdxesUbound=0; m_ImgCount=0; }
	if (m_hImgList) { ImageList_Destroy(m_hImgList); m_hImgList=NULL; }
}

HIMAGELIST CBImageList::CreateImageList( int iImgWidth/*=16*/, int iImgHeight/*=16*/, EImageListFlagsColor colorType/*=imlcol_Color32*/, bool fUseMask/*=true*/, int imgCountInit/*=128*/, int imgCountMax/*=8192 */ )
{
	UINT flags = (UINT)colorType;

	// 先销毁之前的 ImageList 控件（如果有）
	if (m_hImgList) { ImageList_Destroy(m_hImgList); m_hImgList=NULL; }

	// 创建 ImageList 控件
	if (fUseMask) flags = flags | ILC_MASK;
	m_hImgList = ImageList_Create(iImgWidth, iImgHeight, flags, imgCountInit, imgCountMax);
	return m_hImgList;
}

HIMAGELIST CBImageList::hImageList()
{
	if (m_hImgList==NULL) CreateImageList();   // 如还没创建 ImageList 控件，按默认设置自动创建一个
	return m_hImgList;
}

int CBImageList::ImageIdx( int iOrderNum )
{
	if (iOrderNum > 0 && iOrderNum <= m_ImgCount) 
		return m_ImgIdxes[iOrderNum];
	else
		return 0;
}

int CBImageList::ImageCount()
{
	return m_ImgCount;
}

int CBImageList::AddImageIcon( unsigned short iconRes )
{
	HICON hIcon = LoadIcon(pApp->hInstance, (LPCTSTR)((DWORD)iconRes));
	return AddImageIcon(hIcon);
}

int CBImageList::AddImageIcon( EStandardIcon icon )
{
	HICON hIcon = LoadIcon(0, (LPCTSTR)icon);
	return AddImageIcon(hIcon);
}

int CBImageList::AddImageIcon( HICON hIcon )
{
	const int c_imgIdxesExpandPer = 128;  // m_ImgIdxes[] 空间不够用时，每次动态扩大的大小
	int idxR = ImageList_ReplaceIcon(m_hImgList, -1, hIcon) + 1;
	// If i is -1, the function appends the image to the end of the list.
	if (idxR)
	{
		// 将 idxR 值添加到数组 m_ImgIdxes[ ]
		m_ImgCount++;
		if (m_ImgCount > m_ImgIdxesUbound)
		{
			// 新开辟空间
			int * pNew = new int [m_ImgIdxesUbound + c_imgIdxesExpandPer + 1];
			pNew[0] = 0;	// 0号元素空间赋值为0（不使用；下标将从1开始）
			if (m_ImgIdxes)	// 拷贝原空间数据到新空间
			{
				for (int i=1; i<=m_ImgIdxesUbound; i++) pNew[i]=m_ImgIdxes[i];
				delete []m_ImgIdxes; m_ImgIdxes=NULL;
			}
			m_ImgIdxesUbound += c_imgIdxesExpandPer;
			m_ImgIdxes = pNew;			
		}
		m_ImgIdxes[m_ImgCount] = idxR;
	}
	return idxR;
}
