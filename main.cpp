...
void PerformCalculation() {
    ...
    if (ctPath > 0) {
        ...
        tstring sMsg = TEXT("[OK] 计算完成！最短距离: ");
        sMsg += Str(distance);
        sMsg += TEXT(" | 经过 ");
        sMsg += Str(ctPath);
        sMsg += TEXT(" 个节点");
        ...
        UpdateStatus(TEXT("[失败] 计算完成：两点之间无路径。"));
...
void cmdCheck_Click() {
    ...
    if (nErr > 0 || nSelfLoop > 0) {
        ...
        UpdateStatus(TEXT("[警告] 数据存在问题"));
    } else {
        MsgBox(TEXT("[OK] 数据正常，未发现问题！"), TEXT("数据检查结果"), mb_OK, mb_IconInformation);
        UpdateStatus(TEXT("[OK] 数据检查通过"));
    }
}
...
void cmdCopyResult_Click() {
    ...
    UpdateStatus(TEXT("[OK] 结果已复制到剪贴板"));
}
